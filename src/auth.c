...
int Authentication(const char *UserName, const char *Password, const char *DeviceName)
{
	...
	printf("MAC Addr:%02x:%02x:%02x:%02x:%02x:%02x",MAC[0],MAC[1],MAC[2],MAC[3],MAC[4],MAC[5]);
#endif//_Y_DEBUG
	/*
	 * 设置过滤器：
...
//		if ((EAP_Type)captured[22] == NOTIFICATION)
		if ((EAP_Type)captured[22] == IDENTITY)//add by sky
		{
			DPRINTF("[%d] Server: Request identify!\n", captured[19]);
			// 发送Response Notification
			//SendResponseNotification(adhandle, captured, ethhdr);
			GetIpFromDevice(ip, DeviceName);//add by sky
			SendResponseIdentity(adhandle, captured, ethhdr, ip, UserName);//add by sky
			DPRINTF("    Client: Response identify.\n");
...
//		if ((EAP_Type)captured[22] == IDENTITY)
		if ((EAP_Type)captured[22] == MD5)//add by sky
		{	// 通常情况会收到包Request Identity，应回答Response Identity
			DPRINTF("[%d] Server: Request md5!\n", captured[19]);
//			GetIpFromDevice(ip, DeviceName);
//			SendResponseIdentity(adhandle, captured, ethhdr, ip, UserName);
			SendResponseMD5(adhandle, captured, ethhdr, UserName, Password);//add by sky
			DPRINTF("[%d] Client: Response md5.\n", (EAP_ID)captured[19]);
		}
		...
		for (;;)
		{
			...
			if ((EAP_Code)captured[18] == FAILURE)
			{	// 处理认证失败信息
			...
			if ((EAP_Code)captured[18] == SUCCESS)
			{
...
//get service information
			if ((EAP_Code)captured[18] == 10)//add by sky
			{
...
af：一个地址描述。目前仅支持AF_INET格式，也就是说ARPA Internet地址格式。在linux中提供了PF_PACKET接口可以操作链路层的数据
...
void SendStartPkt(pcap_t *handle, const uint8_t localmac[])
{
	...
	packet[12] = 0x88;
	packet[13] = 0x8e;
//EAPOL CASE: code,identifiler,length,data
	// EAPOL (4 Bytes)
